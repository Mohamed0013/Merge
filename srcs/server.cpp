...
void Server::acceptNewClient()
{
    ...
    if (clientFd < 0)
    {
        if (errno != EAGAIN && errno != EWOULDBLOCK)
            std::cerr << "[NETWORK] accept() failed: " << strerror(errno) << std::endl;
        return;
    }

    if (fcntl(clientFd, F_SETFL, O_NONBLOCK) < 0)
    {
        std::cerr << "[NETWORK] fcntl() failed on client FD " << clientFd << std::endl;
        close(clientFd); // FIX: don't leak the FD!
        return;
    }
    ...
    {
        std::cerr << "[NETWORK] epoll_ctl ADD failed for FD " << clientFd << std::endl;
        close(clientFd); // FIX: don't leak the FD!
        return;
    }
...
void Server::handleClientRead(int fd)
{
    ...
    if (bytes == 0)
    {
        ...
        removeClient(fd); // use cleanup method
        return;
    }
    if (bytes < 0)
    {
        if (errno == EAGAIN || errno == EWOULDBLOCK)
        {
            return; // No data available right now — normal for non-blocking
        }
        // Real error
        std::cerr << "[NETWORK] recv() error on FD " << fd << std::endl;
        removeClient(fd); // use cleanup method
        return;
    }

    // Protect against infinite buffer growth (no \r\n received)
    if (_clients[fd].getRecvBuffer().length() + bytes > 4096)
    {
        std::cerr << "[NETWORK] Buffer overflow, disconnecting FD " << fd << std::endl;
        removeClient(fd);
        return;
    }
...
void Server::handleClientWrite(int fd)
{
    ...
    if (sent < 0)
    {
        if (errno == EAGAIN || errno == EWOULDBLOCK)
        {
            return; // Socket buffer full — EPOLLOUT will trigger again
        }
        // Real error — disconnect client
        std::cerr << "[NETWORK] send() error on FD " << fd << std::endl;
        removeClient(fd);
        return;
    }
...
void Server::run()
{
    while (_running)
    {
        ...
        if (nfds < 0)
        {
            if (errno == EINTR)
            {
                // Signal interrupted epoll_wait — check _running and continue
                continue;
            }
        ...
        for (int i = 0; i < nfds; ++i)
        {
            ...
            else
            {
                if (_events[i].events & EPOLLIN)
                {
                    ...
                    // NOTE: client may have been removed! Check if still exists.
                ...
                {
                    std::cout << "[NETWORK] Socket error/hangup on FD " << current_fd << std::endl;
                    removeClient(current_fd);
                }
            }
        }
    }
